...
-- 3. Wrap all operations in BEGIN...EXCEPTION...END blocks where applicable
...
BEGIN
    -- Only update profiles that don't have a member_id yet
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            ...
            RAISE NOTICE 'Error updating member_id for profile %: %', profile_record.id, SQLERRM;
        END;
    END LOOP;
END $$;
...
DECLARE
    ride_record RECORD;
BEGIN
    -- Only update rides that don't have a trip_id yet
    FOR ride_record IN 
        SELECT id 
        FROM rides 
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Log error but continue with next record
            RAISE NOTICE 'Error updating trip_id for ride %: %', ride_record.id, SQLERRM;
        END;
    END LOOP;
END $$;
...
-- Output counts of updated records for verification
DO $$
DECLARE
