...
export default function MemberProfilePage({ params }: { params: { id: string } }) {
  ...
  const fetchMemberData = async (memberId: string) => {
    try {
      ...
      const { data: memberData, error: memberError } = await supabase
        ...
        .select(`
          *,
          provider:transportation_providers!profiles_provider_id_fkey(name, organization_code)
        `)
        .eq('id', memberId)
        .single()
  ...
  const setDemoData = (memberId: string) => {
    ...
    const demoMember: Member = {
      id: memberId,
      member_id: `M${100000 + parseInt(memberId.slice(0, 5), 16) % 10000}`,
      full_name: 'John Doe',
    ...
    // Demo ride statuses
    const statuses = ['completed', 'cancelled', 'in_progress', 'scheduled', 'completed']
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="border rounded-lg p-3">
                  ...
                  <div className="text-2xl font-bold mt-1">{rides.length}</div>
                </div>
                <div className="border rounded-lg p-3">
                  <div className="text-sm text-muted-foreground">Active Rides</div>
                  <div className="text-2xl font-bold mt-1">
                    {rides.filter(ride => ride.status === 'scheduled' || ride.status === 'in_progress').length}
                  </div>
                </div>
              </div>
                      ...
                      <tbody>
                        {rides.map((ride) => (
                          <tr key={ride.id} className="border-b hover:bg-muted/50">
                            ...
                            <td className="py-3 px-4">{ride.pickup_location}</td>
                            <td className="py-3 px-4">{ride.dropoff_location}</td>
                            <td className="py-3 px-4">
                              <Badge variant={
                            ...
                            <td className="py-3 px-4">{ride.driver_name || 'Unassigned'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
